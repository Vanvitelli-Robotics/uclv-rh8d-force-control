...
class GainNode : public rclcpp::Node
{
public:
          ...
          motor_ids_(this->declare_parameter<std::vector<int64_t>>("motor_ids", std::vector<int64_t>()))
    {
        ...
        sensor_state_sub_ = this->create_subscription<uclv_seed_robotics_ros_interfaces::msg::FTS3Sensors>(
            "/sensor_state", 1, std::bind(&GainNode::sensor_state_callback, this, std::placeholders::_1));

        desired_forces_sub_ = this->create_subscription<uclv_seed_robotics_ros_interfaces::msg::FTS3Sensors>(
            "/cmd/desired_forces", 1, std::bind(&GainNode::desired_forces_callback, this, std::placeholders::_1));
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        auto gain_node_node = std::make_shared<GainNode>();
        rclcpp::spin(gain_node_node);
    }
